...
The UI for the multiparty audio/video communication is represents by an Android GridLayoutManager with RecycleViews. The different layout distributions depend on the number of participants. To adjust the number of columns for the GridLayout, the `setSpanSizeLookup` method has been override. 
